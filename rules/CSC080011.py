...
def checkSwitchBlock(filename, lines, rawlines, startLineNo, ruleCheckKey, Error, cpplines, fileErrorCount):
  ...
  # error message
  identErrorMessage = "Code Style Rule: Case and default do not need to indent (to avoid excessive indentation)."
  # check
  while i <= switchEndLineNo:
    ...
    if m:
      line = lines[i]
      indentErrorTypeWord = ''
      indentCharactorQtyOfCaseDefault = len(m.group(1).replace('\t','    '))
      if indentCharactorQtyOfSwitch != indentCharactorQtyOfCaseDefault:
        indentError = True
        
        tmp = ''
        j = 0
        while j < indentCharactorQtyOfSwitch:
          tmp = tmp + ' '
          j = j + 1
        cpplines[i] = cpplines[i].replace(m.group(1), tmp)
      ...
      if m.group(2) == 'case':
        ...
        checkCaseBlock(filename, lines, rawlines, i, switchEndLineNo, ruleCheckKey, Error, cpplines, fileErrorCount)
  ...
  if indentError:
    Error(filename, lines, startLineNo, ruleCheckKey, 3, identErrorMessage)
  return i - 1
      
      

def CheckCSC080011(filename, file_extension, clean_lines, rawlines, nesting_state, ruleCheckKey, Error, cpplines, fileErrorCount):
